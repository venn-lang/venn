...
import type { MatcherArgs, MatcherContext } from "./types/context.types.js";
 ...
 * and the `test` / `message` / `detail` hooks. The two that produce text are
 * handed a {@link MatcherContext}, whose `show` is how a value becomes words.
...
export function defineMatcher<S extends ZodType = ZodType>(def: {
  ...
  message(args: MatcherArgs<z.infer<S>>, ctx: MatcherContext): string;
  /** The two sides compared, so the failure carries a diff and not just prose. */
  detail?(args: MatcherArgs<z.infer<S>>, ctx: MatcherContext): MatcherDetail;
}): MatcherDefinition {
