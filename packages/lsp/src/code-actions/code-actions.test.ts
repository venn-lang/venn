...
describe("quick fixes", () => {
  ...
  it("offers the package that provides the namespace, marked preferred", async () => {
    ...
    expect(actions.map((action) => action.title)).toEqual(['Add import { http } from "venn/http"']);
    expect(actions[0]?.isPreferred).toBe(true);
  });

  it("inserts the import after the ones already there", async () => {
  ...
  it("inserts it after an import from a file, too", async () => {
...
flow "F" {
  step "s" { http.get "https://example.com" }
    ...
    const updated = applyEdits(document, edits);
    const added = updated.indexOf('import { http } from "venn/http"');
...
describe("quick fix for env inside a string", () => {
  ...
  it('offers `import { env } from "venn/env"` for an env read hidden in a placeholder', async () => {
    const { actions } = await fixesFor(ENV_IN_STRING);

    expect(actions.map((action) => action.title)).toContain('Add import { env } from "venn/env"');
  });
});
